...
//static float u_hover[4] = {0.653f, 0.741f, 0.676f, 0.732f};  // CF Brushless lowbat
static float u_hover[4] = {0.6355f, 0.6760f, 0.6507f, 0.6840f};
...
// My variables for time recording
static uint32_t mpc_exec_time_us = 0;
...
void controllerOutOfTreeInit(void) { 
  ...
  #include "my_params_100hz.h"  // My gains
...
void controllerOutOfTree(control_t *control, const setpoint_t *setpoint, const sensorData_t *sensors, const state_t *state, const uint32_t tick) {
  ...
  // Calcula o tempo total de execução em microssegundos
  mpc_exec_time_us = (uint32_t)(usecTimestamp() - startTimestamp);
}
...
LOG_GROUP_START(ctrlMPC)
  // Deixe as variáveis que você precisa monitorar aqui
  //LOG_ADD(LOG_UINT8, result, &result) 
  // Aqui está a variável de tempo de processamento do TinyMPC
  LOG_ADD(LOG_UINT32, exec_time, &mpc_exec_time_us)
LOG_GROUP_STOP(ctrlMPC)
